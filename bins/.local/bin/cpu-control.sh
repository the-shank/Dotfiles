...
# CREDITS:
# https://github.com/wlk/cpu-control
...
if [ "$EXPECTED_ENABLED_CPU_COUNT" -lt 1 ]; then
...
#
if [ "$ENABLED_CPU_COUNT" -eq "$EXPECTED_ENABLED_CPU_COUNT" ]; then
  echo "Enabled CPU count matches expected enabled CPU count, not doing anything"
elif [ "$ENABLED_CPU_COUNT" -gt "$EXPECTED_ENABLED_CPU_COUNT" ]; then
...
for CPU_ID in $(seq 1 $(($TOTAL_CPU_COUNT - 1))); do
  ...
  if [ "$CPU_ENABLED" -eq 1 ]; then
    CPU_STATUS="enabled"
    if [ "$CPU_COUNT_TO_DISABLE" -gt 0 ]; then
      CPU_COUNT_TO_DISABLE=$((CPU_COUNT_TO_DISABLE - 1))
      echo 0 >"$CPU_PATH"
    fi
  else
    CPU_STATUS="disabled"
    if [ "$CPU_COUNT_TO_ENABLE" -gt 0 ]; then
